...
function allowedFramers() {
  return ["http://localhost:5173","http://127.0.0.1:5173","https://antot-12.github.io"].join(" ");
}
function stripMetaCsp(html) { return html.replace(/<meta[^>]+http-equiv=["']content-security-policy["'][^>]*>/ig, ""); }
...
function rewriteCssUrls(css, baseHref) {
  return css.replace(/url\((['"]?)(?!data:|https?:|blob:|#)([^'")]+)\1\)/gi, (_m, q, u) => {
    let abs;
    try { abs = new URL(u, baseHref).href; } catch { abs = u; }
...
function ensureStyles(html) {
  const inject = `<style>html,body{min-height:100%}img,video{max-width:100%}</style>`;
...
export async function createApp() {
  ...
  app.use(cors({ origin: ["http://localhost:5173","http://127.0.0.1:5173","https://antot-12.github.io"] }));
  app.use(helmet());
  app.use(compression());
  app.use(morgan("tiny"));

  const DATA = process.env.VERCEL ? "/tmp" : join(process.cwd(), "data");
  const SAFE = process.env.SAFE_DIR || join(DATA, "sanitized");
  ...
  app.get("/api/history", (req, res) => {
    ...
    if (["low","medium","high"].includes(risk)) items = items.filter(it => (it.risk?.label||"").toLowerCase()===risk);
    if (sort==="risk") items.sort((a,b)=>(b.risk?.score||0)-(a.risk?.score||0));
  ...
  app.post("/api/analyze", async (req, res) => {
    ...
    try {
      ...
      if (/text\/html/i.test(contentType)) {
        ...
        const t = html.match(/<title>([^<]{0,200})<\/title>/i); if (t) title = t[1].trim();
  ...
  app.get("/api/sanitized/:id", async (req, res) => {
    const file = join(process.env.VERCEL ? "/tmp" : join(process.cwd(),"data"), "sanitized", `${req.params.id}.html`);
    try {
